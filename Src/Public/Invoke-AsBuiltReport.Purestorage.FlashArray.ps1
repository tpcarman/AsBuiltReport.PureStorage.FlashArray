function Invoke-AsBuiltReport.PureStorage.FlashArray {
    ...
    Write-PScriboMessage -IsWarning "Please refer to the AsBuiltReport.PureStorage.FlashArray github website for more detailed information about this project."
    Write-PScriboMessage -IsWarning "Do not forget to update your report configuration file after each new version release."
    Write-PScriboMessage -IsWarning "Documentation: https://github.com/AsBuiltReport/AsBuiltReport.PureStorage.FlashArray"
    Write-PScriboMessage -IsWarning "Issues or bug reporting: https://github.com/AsBuiltReport/AsBuiltReport.PureStorage.FlashArray/issues"

    # Check the current AsBuiltReport.PureStorage.FlashArray module
    Try {
        $InstalledVersion = Get-Module -ListAvailable -Name AsBuiltReport.PureStorage.FlashArray -ErrorAction SilentlyContinue | Sort-Object -Property Version -Descending | Select-Object -First 1 -ExpandProperty Version

        if ($InstalledVersion) {
            Write-PScriboMessage -IsWarning "AsBuiltReport.PureStorage.FlashArray $($InstalledVersion.ToString()) is currently installed."
            $LatestVersion = Find-Module -Name AsBuiltReport.PureStorage.FlashArray -Repository PSGallery -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Version
            if ($LatestVersion -gt $InstalledVersion) {
                Write-PScriboMessage -IsWarning "AsBuiltReport.PureStorage.FlashArray $($LatestVersion.ToString()) is available."
                Write-PScriboMessage -IsWarning "Run 'Update-Module -Name AsBuiltReport.PureStorage.FlashArray -Force' to install the latest version."
            }
        }
    } Catch {
    ...
    foreach ($System in $Target) {
        ...
        if ($Array) {
            ...
            #>
            $Pfa2Array = Get-Pfa2Array -Array $Array
            Section -Style Heading1 $Pfa2Array.Name {
                if ($InfoLevel.System -ne 0) {
                    Get-AbrPfa2Array
                }
